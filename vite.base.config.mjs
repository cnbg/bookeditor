...
export const getBuildConfig = (env) => {
    ...
    return {
        root, mode, build: {
            ...
            emptyOutDir: false, // 🚧 Multiple builds may conflict.
            outDir: '.vite/build', watch: command === 'serve' ? {} : null, minify: command === 'build',
        }, clearScreen: false,
    }
}
...
export const getDefineKeys = (names) => {
    ...
    return names.reduce((acc, name) => {
        ...
        const keys = {
            VITE_DEV_SERVER_URL: `${NAME}_VITE_DEV_SERVER_URL`, VITE_NAME: `${NAME}_VITE_NAME`,
        }
...
export const pluginExposeRenderer = (name) => {
    ...
    return {
        name: '@electron-forge/plugin-vite:expose-renderer', configureServer(server) {
...
export const pluginHotRestart = (command) => {
    return {
        name: '@electron-forge/plugin-vite:hot-restart', closeBundle() {
            if (command === 'reload') {
